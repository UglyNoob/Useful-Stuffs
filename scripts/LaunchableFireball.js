...
world.events.entitySpawn.subscribe((event) => {
	if(event.entity.typeId == "minecraft:snowball") {
		let location = event.entity.location;
		let velocity = event.entity.getVelocity();
		let dimension = event.entity.dimension;
		event.entity.kill();

		location.x += velocity.x;
		location.y += velocity.y;
		location.z += velocity.z;
		let fireball = dimension.spawnEntity("minecraft:fireball", location);
		fireball.applyImpulse(velocity);
		fireball.age = fireballLastingTicks;
		fireball.addTag(fireballTag);
	}
